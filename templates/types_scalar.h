...
% elif ty.category == ty.BASETYPE:
    return vn_size_${ty.typedef.name}(val); /* typedef */
% elif ty.category == ty.ENUM:
    assert(sizeof(*val) == sizeof(int32_t));
    return sizeof(int32_t); /* enum */
% endif
</%def>
...
%   endif
% elif ty.category == ty.BASETYPE:
    vn_encode_${ty.typedef.name}(cs, val); /* typedef */
% elif ty.category == ty.ENUM:
    vn_encode_int32_t(cs, (const int32_t *)val); /* enum */
% endif
</%def>
...
%   endif
% elif ty.category == ty.BASETYPE:
    vn_decode_${ty.typedef.name}(cs, val); /* typedef */
% elif ty.category == ty.ENUM:
    vn_decode_int32_t(cs, (int32_t *)val); /* enum */
% endif
</%def>
